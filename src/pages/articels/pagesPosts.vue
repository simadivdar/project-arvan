      ...
      <nav aria-label="Page navigation example" class="d-flex justify-content-center mt-5">
        <ul class="pagination  pagination-lg">
            <li  class="page-item" v-if="pagechange>1">
              <a class="page-link"  @click="pagechange--,setPage(pagechange),$router.replace({ params: { page: pagechange } }),changePage()"  aria-label="Previous">
                <span aria-hidden="true">&laquo;</span>
              </a>
            </li>
            <li class="page-item"  v-for="page,index in pageNumber " :key="index">
              <a class="page-link" @click="setPage(page) , $router.replace({ params: { page: page } }),changePage()">
                {{ page }}
              </a>
            </li>
            <li class="page-item" v-if="pagechange<pageNumber">
              <a class="page-link"  @click="pagechange++,setPage(pagechange),changePage(),$router.replace({ params: { page: pagechange } })"  aria-label="Next">
...
<script>
import {setSHow,textSHow,setPosts,setPage,reloadPage} from "@/services/Postservice.js";
import Table from "@/components/tableart.vue";
import {ref } from "vue";
...
setup(){
  ...
  return{Articels,pageNumber,changePage,router,pagechange,textshow,show,setPage,page,reloadPage}
}
}
</script>
